...
public class MyConnector {
	 ...
	 * "C:\\Users\\pdidi\\.m2\\repository\\net\\tirasa\\connid\\bundles\\db\\net.tirasa.connid.bundles.db.table\\2.2.6"
	 * ); File bundleDirectory = new File(file); URL url = null; try { //url =
	 * IOUtil.makeURL(bundleDirectory,
	 * "net.tirasa.connid.bundles.db.table-2.2.6.jar"); url =
	 * IOUtil.makeURL(bundleDirectory, jar); } catch (IOException e) { // TODO
	...
	/*
	 * //System.out.println(url); ConnectorInfoManager manager =
	 * fact.getLocalManager(url); //ConnectorKey key = new
	 * ConnectorKey("net.tirasa.connid.bundles.db.table", "2.2.6",
	 * "net.tirasa.connid.bundles.db.table.DatabaseTableConnector"); ConnectorKey
	 * key = new ConnectorKey(bundle,version,connidType);
	 * 
	 * //System.out.println(key); //System.out.println(manager);
	 * 
	 ...
	 * 
	 * // Print out what the properties are (not necessary) //List<String>
	 * propertyNames = properties.getPropertyNames();
	 * 
	 * 
	 * //works String host = "127.0.0.1"; properties.setPropertyValue("host", host);
	 * String port = "3306"; properties.setPropertyValue("port", port); String user
	 * = "root"; properties.setPropertyValue("user", user); GuardedString password =
	 * new GuardedString("root".toCharArray());
	 * properties.setPropertyValue("password", password); String database =
	 * "monokee"; properties.setPropertyValue("database", database); String table =
	 * "accounts"; properties.setPropertyValue("table", table); String keyColumn =
	 * "accountId"; properties.setPropertyValue("keyColumn", keyColumn); String
	 * driver = "com.mysql.cj.jdbc.Driver";
	 * properties.setPropertyValue("jdbcDriver", driver); String urltemplate =
	 * "jdbc:mysql://%h:%p/%d"; properties.setPropertyValue("jdbcUrlTemplate",
	 * urltemplate); String passwordCharset = "UTF-8";
	 * properties.setPropertyValue("passwordCharset", passwordCharset);
	 * 
	 * //properties.setPropertyValue(name, value); // Use the
	 * ConnectorFacadeFactory's newInstance() method to get a new connector.
	 * ConnectorFacade connector =
	 * ConnectorFacadeFactory.getInstance().newInstance(apiConfig);
	 * 
	 * // Make sure we have set up the Configuration properly connector.validate();
	 * 
	 * conn = connector; }
	 */
